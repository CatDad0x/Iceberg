...
// CLINICAL. minimal personality. Penguin only in header.
// Warm tan palette, but report body stays professional.

const BG = "bg-[#F8F1E2]";
const SURFACE = "bg-white";
const PRIMARY = "#C8302B"; // penguin red
const INK = "text-[#1B1B2F]";
const MUTED = "text-[#7A6B58]";

export default function ClinicalMock() {
  return (
...
function Header() {
  return (
    <div className="flex items-center justify-between pb-4">
      <Link href="/mock" className={`flex items-center gap-3 ${INK}`}>
        <Image src="/cat-dad.png" alt="" width={40} height={40} className="rounded-lg" />
        <div>
          <div className="text-lg font-bold leading-none">Iceberg</div>
          <div className={`text-[11px] ${MUTED}`}>Iceberg Risk Tool · DeFi position analyzer</div>
        </div>
      </Link>
      <a
...
function StickyBar() {
  return (
    <div className="sticky top-0 z-40 border-b border-[#E8DCC4] bg-[#F8F1E2]/90 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-2.5 text-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className={`font-semibold ${INK}`}>Low Risk</span>
            <span className={`ml-2 ${MUTED}`}>
              {MOCK.protocol} · {MOCK.pair} · Base
            </span>
          </div>
        </div>
  ...
  return (
    <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
      ...
      <Card>
        ...
        <a href={MOCK.explorerTxUrl} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline">
          View transaction ↗
        </a>
      </Card>
    </div>
        ...
        <div className="px-4 pb-4 pl-13">
          ...
          {check.learnMoreUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="ml-9 mt-2 inline-block text-xs text-blue-600 hover:underline"
            >
              Learn more →
            </a>
          )}
        ...
        {MOCK.assets.map((a) => (
          <div key={a.address} className="border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="ml-8 mt-1 inline-block font-mono text-[11px] text-blue-600 hover:underline"
            >
...
function Footer() {
  return (
    <div className={`mt-12 flex items-center justify-between border-t border-[#E8DCC4] pt-6 text-xs ${MUTED}`}>
      <span>Iceberg Risk Tool · Not financial advice · Verify all findings independently</span>
      <a href="https://x.com/CatDad0x" target="_blank" rel="noreferrer" className={`hover:text-[${PRIMARY}]`}>
        Built by @CatDad0x
      </a>
    </div>
  );
